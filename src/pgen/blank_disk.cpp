...
Real z_disk = 0.2;
Real v_disk; 
Real rho_disk = 2.e5*icm3_to_code*mh2;
Real temp_disk = 3.e3;
// Outflow winds
bool outflow = true;
Real r_wind = 0.5;
Real v_wind = 7.e7*cm_to_pc/s_to_kyr;
Real rho_wind = 1.e2*icm3_to_code*mh2;
Real temp_wind = 1.e6;
// Ambient medium
Real rho_amb = 1.*icm3_to_code*mh2;
Real temp_amb = 1.e6;
// Problem 
Real pfloor, rhofloor;
Real vmax = 3.e8*cm_to_pc/s_to_kyr;
Real GM;
...
#pragma simd
      for (int i=pmb->is; i<=pmb->ie; ++i) {
       ...
       /* Add the outflow winds*/
        if (outflow){
          Real vx_wind, vy_wind, vz_wind; 
          Real press_wind;
          if (r < r_wind && r > r_inner_boundary){
            vx_wind = v_wind*x/r;
            vy_wind = v_wind*y/r;
            vz_wind = v_wind*z/r;
            press_wind = rho_wind*kb*temp_wind/mh2;            

            prim(IDN,k,j,i) = rho_wind;
            prim(IVX,k,j,i) = vx_wind;
            prim(IVY,k,j,i) = vy_wind;
            prim(IVZ,k,j,i) = vz_wind;
            prim(IPR,k,j,i) = press_wind;
          }
        }
  ...
  for (k=ks-NGHOST; k<=ke+NGHOST; k++){
    for (j=js-NGHOST; j<=je+NGHOST; j++){
      for (i=is-NGHOST; i<=ie+NGHOST; i++){
        ...
        v_disk = std::sqrt(GM/R);
        theta = std::atan2(y,x);
        int_energy = rho_disk*kb*temp_disk / ((gas_gamma - 1)* mh2);
        // Add in the Blank Disk
        if (R >= r_in_disk && R <= r_out_disk && std::abs(z) < z_disk){
          phydro->u(IDN,k,j,i) = rho_disk;
          phydro->u(IM1,k,j,i) = rho_disk*v_disk*std::cos(theta);
          phydro->u(IM2,k,j,i) = rho_disk*v_disk*std::sin(theta);
          phydro->u(IM3,k,j,i) = 0;
          phydro->u(IEN,k,j,i) = (0.5*rho_disk)*SQR(v_disk) + int_energy;
        }
        // Assign ambient parameters everywhere else
          ...
          phydro->u(IM3,k,j,i) = 0;
          phydro->u(IEN,k,j,i) = rho_amb*kb*temp_amb / ((gas_gamma -  1)* mh2);
        }
      }
    }
  }
