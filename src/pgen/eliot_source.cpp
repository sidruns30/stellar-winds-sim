...
int N_user_vars = 2; /* Number of user defined variables in UserWorkAfterLoop */
...
Real cs_max = 0.023337031 * cl;  /*sqrt(me/mp) cl....i.e. sound speed of electrons is ~ c */
...
#pragma simd
      ...
      get_cartesian_coords(pmb->pcoord->x1v(i), pmb->pcoord->x2v(j), pmb->pcoord->x3v(k), &x, &y, &z);
      if (pmb->block_size.nx3 >1 || COORDINATE_SYSTEM == "spherical_polar") {
        r = sqrt(SQR(x) + SQR(y) + SQR(z));
        v = sqrt( SQR(prim(IVX,k,j,i)) + SQR(prim(IVY,k,j,i)) + SQR(prim(IVZ,k,j,i)) );
        if (eta == 3) A = mdot/(4.*PI) * 1./(log(rout/rin));
        else A = mdot * (3. - eta)/(4. * PI) * 1. / (pow(rout,3.-eta) - pow(rin,3.-eta));
      }
      else if (pmb->block_size.nx2 >1 || COORDINATE_SYSTEM == "cylindrical") {
        eta = 1;
        ...
        if (eta == 1) A = mdot/(log(rout/rin));
          ...
          if ( (r>=rin) && (r<=rout) ){
              ...
              cons(IEN,k,j,i) += 0.5 * d0 * ( SQR(vwind) + SQR(vorbit)); 
          }
...
void MeshBlock::InitUserMeshBlockData(ParameterInput *pin){
    ...
    if (COORDINATE_SYSTEM == "spherical_polar" || COORDINATE_SYSTEM == "cylindrical") r_inner_boundary = 0;
...
#pragma simd
            for (int i=is; i<=ie; ++i) {
                ...
                Real r = sqrt( SQR(x) + SQR(y)*is_2D + SQR(z)*is_3D);
