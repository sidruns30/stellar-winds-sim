...
// More parameters
Real C_disk, K_disk, P_amb, A_disk; 
...
Real centrifugal(Real R){
  return SQR(C_disk) * std::pow(R, -2*q + 2) / (2*q - 2);
}

// Integration constant 
Real integration_cons(){
  Real t1 = centrifugal(R_in);
  Real t2 = gas_gamma * P_amb*std::pow(K_disk, 1/gas_gamma) / ((gas_gamma - 1) * std::pow(P_amb, 1/gas_gamma));
...
Real disk_press(Real r_vec[3]){
  ...
  Real factor = (gas_gamma - 1) / (gas_gamma * std::pow(K_disk, 1/gas_gamma));
  Real terms  = -(A_disk + potential(r) + centrifugal(R));
  // To avoid nans:
  if (terms < 0){
    terms = 0;
  }
            ...
            if (x<box_radius && x > -box_radius && y<box_radius
              ...
              if (current_level < n_level){
                  ...
                  if (new_r_in<pmb->r_inner_boundary) {
                    ...
                    if (pmb->r_inner_boundary < horizon_radius) pmb->r_inner_boundary = horizon_radius;
...
#pragma simd
  ...
  */
  ...
  Real torus_rho = std::pow(press/K_disk, 1/gas_gamma);
  Real int_energy = press / (gas_gamma - 1);
  Real torus_omega = C_disk * std::pow(R, -q);
  ...
  // Initialize torus after winds reach equilibrium
  if(std::abs(time-t_blob)<dt){      
    if (R >= 1.3 && R<2.8){
      cons(IDN,k,j,i) = torus_rho;
      cons(IM1,k,j,i) = -torus_rho * torus_v * std::sin(phi);
      cons(IM2,k,j,i) = torus_rho * torus_v * std::cos(phi);
      cons(IM3,k,j,i) = 0;
      cons(IEN,k,j,i) = (0.5 * torus_rho) * SQR(torus_v) + int_energy;
    }
    /* Position Arm 1
...
void Mesh::InitUserMeshData(ParameterInput *pin)
{
  ...
  C_disk = std::sqrt(GM) * std::pow(R_k, q-1.5);
  K_disk = (rho_tar * kb * T_tar) / (mh2 * std::pow(rho_tar, gas_gamma));
  P_amb = pfloor;
  A_disk = integration_cons();
