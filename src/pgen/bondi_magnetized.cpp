...
Real beta_inf;  /* beta at infinity for bondi */
 ...
 void cons_force(MeshBlock *pmb,const Real time, const Real dt,const AthenaArray<Real> &prim, const AthenaArray<Real> &bcc, AthenaArray<Real> &cons);
  ...
  void get_Bondi_3D(const Real r, Real *rho, Real *v, Real *a );
  void get_B(const Coordinates *pcoord, const int i, const int j, const int k, Real *bx1f, Real *bx2f, Real *bx3f);
  void apply_bondi_boundary(MeshBlock *pmb,AthenaArray<Real> &prim);
...
#pragma simd
      for (int i=pmb->is; i<=pmb->ie; ++i) {
          ...
          //fprintf(stderr,"Is this called? \n xyz: %g %g %g\n r: %g r_inner: %g\n", x,y,z,r,r_inner_boundary);
...
//              }
             ...
             // }
              ...
              Real bsq_rho_ceiling = SQR(v_ff);
              Real bsq = SQR(pmb->pfield->bcc(IB1,k,j,i)) + SQR(pmb->pfield->bcc(IB2,k,j,i)) + SQR(pmb->pfield->bcc(IB3,k,j,i));
              Real new_rho = bsq/bsq_rho_ceiling;
              Real rho_flr = 1e-8;
              Real p_floor = 1e-10;

              if (new_rho<rho_flr) new_rho = rho_flr;
              //if (new_rho>prim(IDN,k,j,i)) new_rho = prim(IDN,k,j,i);
              prim(IDN,k,j,i) = new_rho;
              prim(IVX,k,j,i) = 0.;
              prim(IVY,k,j,i) = 0.;
              prim(IVZ,k,j,i) = 0.;
...
#pragma simd
      for (int i=pmb->is; i<=pmb->ie; ++i) {
          ...
          if (r <= r_inner_boundary_bondi || r>= r_outer_boundary_bondi){
              ...
              if (is_3D) get_Bondi_3D(r,&rho,&v,&a);
              else get_Bondi(r,&rho,&v,&a);
              // Real bxa,bya,bza;
              // get_B(pmb->pcoord,i,j,k, &bxa, &bya, &bza);
              ...
              if (COORDINATE_SYSTEM == "cartesian"){
                  prim(IVX,k,j,i) = v*  x/r;
                  prim(IVY,k,j,i) = v*  y/r;
                  if (is_3D) prim(IVZ,k,j,i) = v*  z/r;

              }
              else if (COORDINATE_SYSTEM == "cylindrical"){
                  Real v_phi = l_bondi/std::sqrt(SQR(x) + SQR(y) + 1e-20);
                  prim(IVX,k,j,i) = v;
                  prim(IVY,k,j,i) = v_phi;
                  v = std::sqrt(SQR(v) + SQR(v_phi));
              }
              else{
                Real v_phi = l_bondi/std::sqrt(SQR(x) + SQR(y) + 1e-20);
                prim(IVX,k,j,i) =  v;
                prim(IVZ,k,j,i) = v_phi;
                v = std::sqrt(SQR(v) + SQR(v_phi));
              }
...
//           // if (std::fabs( (pmb->pcoord->x1v(i) - 0.0390625) )<1e-4 && std::fabs( (pmb->pcoord->x2v(j) +0.1015625) )<1e-4 ){
...
Real v_of_a_3D( const Real a, const Real r){
    return -qs /SQR(r) * pow(a,(-2./(gm1)));
  }
Real a_of_x_3D(const Real x,const Real r){
    //#v/a = x -> -qs/r * (a) **(-2/(gam-1) -1.)
    Real x_pow = -2./(gm1) -1.;
...
static Real MachResidual_3D(Real x, Real r)
{

    Real gam = gm1+1.;

    Real a = a_of_x_3D(x,r);
    Real v = v_of_a_3D(a,r);
    return SQR(v)/2. + SQR(a)/(gm1) -1./r - 1./(gm1) ;
...
static Real MachBisect_3D(Real r, Real x_min, Real x_max)
{
  // Parameters
  const int max_iterations = 100;
  const Real tol_residual = 1.0e-8;
  ...
  // Find initial residuals
  Real res_min = MachResidual_3D(x_min, r);
  Real res_max = MachResidual_3D(x_max, r);
  if (std::abs(res_min) < tol_residual) {
    return x_min;
  }
  if (std::abs(res_max) < tol_residual) {
  ...
  for (int i = 0; i < max_iterations; ++i) {
    ...
    Real res_mid = MachResidual_3D(x_mid, r);
    if (std::abs(res_mid) < tol_residual) {
      return x_mid;
    }
    if ((res_mid < 0.0 and res_min < 0.0) or (res_mid > 0.0 and res_min > 0.0)) {
...
void get_Bondi_3D(const Real r, Real *rho, Real *v, Real *a ){
   ...
   Real gam =gm1+1.;
   Real r_scaled = r/r_a;
   qs = 0.25 * pow( 2./(5.-3.0*gam) ,  (5.0-3.0*gam)/(2.*gam-2.)   );
   Real mdot  = 4.*PI * qs;
   Real rs = (5.0-3.0*gam)/4.;
   Real x;

   if (r_scaled<= rs){
    x = MachBisect_3D(r_scaled,1.,1e2);
   }
   else{
    x = MachBisect_3D(r_scaled,1e-10,1.);
   }

   *a = a_of_x_3D(x,r_scaled) * a_inf;
   *v = v_of_a_3D(*a/a_inf,r_scaled) * a_inf;
...
void get_B(const Coordinates *pcoord, const int i, const int j, const int k, Real *bx1f, Real *bx2f, Real *bx3f){
    ...
    AthenaArray<Real> az;
    az.NewAthenaArray(2,2);


  Real UnitB = a_inf * std::sqrt(4. * PI* rho_inf);
  Real gam = gm1 + 1.0;
  Real P_inf = a_inf / gam * rho_inf;

  Real B_inf = std::sqrt(2.0 * P_inf / beta_inf) ;
  ...
  for (int di=0; di<=1; ++di) {
    ...
    Real azimuthal_angle = std::atan2(pcoord->x2f(j+dj),pcoord->x1f(i+di));
    az(dj,di) = B_inf *  pcoord->x2f(j+dj); //std::sin(azimuthal_angle) * r_a;
  }}


  // initialize interface B

  // *bx1f = (az(1,0) - az(0,0))/pcoord->dx2f(j);
  // *bx2f = (az(0,0) - az(0,1))/pcoord->dx1f(i);
  // *bx3f = 0.0;
...
void MeshBlock::InitUserMeshBlockData(ParameterInput *pin){
    ...
    beta_inf = pin->GetOrAddReal("problem","beta_inf",1e20);
    ...
    r_inner_boundary_bondi = 0; //r_inner_boundary;
...
void MeshBlock::ProblemGenerator(ParameterInput *pin)
{
  ...
  //   Real azimuthal_angle = std::atan2(pcoord->x2f(j),pcoord->x1f(i));
  //   az(j,i) = B_a * std::sin(azimuthal_angle) * r_a;
  // }}
  ...
  for (i=il; i<=iu; i++) {
    ...
    if ((block_size.nx3)>1) get_Bondi_3D(r,&rho,&v,&a);
    else get_Bondi(r,&rho,&v,&a);
    get_B(pcoord,i,j,k, &bxa, &bya, &bza);
    //v = 0;
