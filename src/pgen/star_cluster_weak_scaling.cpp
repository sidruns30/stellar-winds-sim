...
void update_star(Stars *star, int i_star, const Real t)
{
    ...
    star[i_star].x1 = x1mid;
    star[i_star].x2 = x2mid;
    star[i_star].x3 = x3mid;
    star[i_star].v1 = 0;
    star[i_star].v2 = 0;
    star[i_star].v3 = 0;
	...
	*vsq_avg = 0.;
	...
	double x,y,z;
        ...
        if (MAGNETIC_FIELDS_ENABLED){
          tmp += std::sin(2.)* starmask(r,star[i_star].radius) * dV;
          tmp += std::sin(2.)* starmask(r,star[i_star].radius) * dV;
          tmp += std::sin(2.)* starmask(r,star[i_star].radius) * dV; 
        }
			}
...
void MeshBlock::InitUserMeshBlockData(ParameterInput *pin){
    ...
    x1mid =  (block_size.x1max-block_size.x1min)/2.;
    x2mid =  (block_size.x2max-block_size.x2min)/2.;
    x3mid =  (block_size.x3max-block_size.x3min)/2.;
