...
Real mu_highT = 1./(2.*X + 3.*(1.-X-Z)/4. + Z/2.);  //mean molecular weight in proton masses
 ...
 void cons_force(MeshBlock *pmb,const Real time, const Real dt,const AthenaArray<Real> &prim, const AthenaArray<Real> &bcc, AthenaArray<Real> &cons);
  ...
  void get_torus(const Real r, const Real theta,Real *rho, Real *v, Real *a );
  void get_Bondi(const Real r, Real *rho, Real *v, Real *a );
void get_Bondi_boundary(const Real r_scaled, Real *rho_scaled, Real *vr_scaled, Real *a_scaled,const Real mdot,const Real kappa,const Real bernoulli,const Real l);
...
Real r_inner_boundary_torus = 0.;
Real r_outer_boundary_torus;
...
int N_user_vars = 5; /* Number of user defined variables in UserWorkAfterLoop */
...
std::string PROBLEM_TYPE;  /* set in input file.  Bondi or Torus */
...
/*
Convert a vector in Cartesian coordinates to spherical polar
*/
void convert_cartesian_vector_to_spherical_polar(const Real vx, const Real vy, const Real vz, const Real x, const Real y, const Real z, Real *vr, Real *vth, Real *vph)
{
    Real r = std::sqrt( SQR(x) + SQR(y) + SQR(z) );
    Real theta = std::acos(z/r);
    Real phi = std::atan2(y,x);

    Real x_hat_r = std::sin(theta)*std::cos(phi);
    Real y_hat_r = std::sin(theta)*std::sin(phi);
    Real z_hat_r = std::cos(theta);

    Real x_hat_theta = std::cos(theta)*std::cos(phi);
    Real y_hat_theta = std::cos(theta)*std::sin(phi);
    Real z_hat_theta = -std::sin(theta);

    Real x_hat_phi = -std::sin(phi);
    Real y_hat_phi = std::cos(phi);
    Real z_hat_phi = 0.;



    *vr =  vx * x_hat_r + vy * y_hat_r + vz * z_hat_r;
    *vth = vx * x_hat_theta + vy * y_hat_theta + vz * z_hat_theta;
    *vph = vx * x_hat_phi + vy * y_hat_phi + vz * z_hat_phi;
}

void convert_spherical_polar_vector_to_cartesian(const Real vr, const Real vth, const Real vph, const Real x, const Real y, const Real z, Real *vx1, Real *vx2, Real *vx3)
{
  ...
  Real phi = std::atan2(y,x);
   ...
   Real r_hat_x = std::sin(theta) * std::cos(phi);
   Real r_hat_y = std::sin(theta) * std::sin(phi);
   Real r_hat_z = std::cos(theta);

   Real th_hat_x = std::cos(theta)*std::cos(phi);
   Real th_hat_y = std::cos(theta)*std::sin(phi);
   Real th_hat_z = -std::sin(theta);

   Real phi_hat_x = -std::sin(phi);
   Real phi_hat_y = std::cos(phi);
   Real phi_hat_z = 0.;

   *vx1 = vr * r_hat_x + vth * th_hat_x + vph * phi_hat_x;
   *vx2 = vr * r_hat_y + vth * th_hat_y + vph * phi_hat_y;
   *vx3 = vr * r_hat_z + vth * th_hat_z + vph * phi_hat_z;
...
/*  */
void get_new_prims(Real const r, Real const r_half ,Real const r_bound, Real const p0, Real const rho0, Real const vr0, 
  Real const p_half, Real const rho_half, Real const vr_half, Real const v_perp,Real *p, Real *rho, Real *vr){
    Real dr = r_bound-r;
    Real a_grav = gm_/SQR(r_half);
    Real a_grav_eff = a_grav - SQR(v_perp)/r_half ;
    Real cs = std::sqrt((gm1+1.)*p_half/rho_half ); 

    Real kappa = p0/std::pow(rho0,gm1+1.);

    Real dp =rho_half *dr * (a_grav_eff - 2.* SQR(vr_half)/r_bound) ;
    dp = dp / (1.-SQR(vr_half/cs));

    if (fabs(1. - fabs(vr_half/cs) ) < 1e-3) dp = rho_half * dr * a_grav;

    *p = p0 + dp;

    if ( *p<1e-10 || isnan(*p) ) *p=1e-10;

    *rho = std::pow(*p/kappa,1./(gm1+1.)) ; 
    *vr = vr0 * SQR(r_bound/r) * ( rho0 / *rho) ;
...
void compute_radial_boundary_values(const Real r, const Real r_bound,const Real den, const Real press,const Real vr, const Real v_th, const Real v_phi, Real *rho_new, Real *press_new, Real*vr_new, Real *v_th_new, Real *v_phi_new){
    
        Real dr = r_bound-r;
        Real v_perp = std::sqrt( SQR(v_th) + SQR(v_phi));
        Real gam = gm1 + 1.;
        Real cs = std::sqrt(gam*press/den );
        Real kappa = press/std::pow(den,gam);
        
        Real mdot = 4. * PI * den * vr * SQR(r_bound);
        Real bernoulli = SQR(vr)/2. + SQR(v_perp)/2. + SQR(cs)/gm1 - gm_/r_bound;
        Real l = v_perp * r_bound;
        
        /* scale to keplerian units */
        
        Real v_kep = std::sqrt(gm_/r_bound);
        Real l_kep = v_kep * r_bound;
        
        Real mdot_unit = den * v_kep * SQR(r_bound);
        Real bernoulli_unit =  SQR(v_kep);
        Real kappa_unit = (den* SQR(v_kep))/std::pow(den,gam);
    
        Real rho_tmp,a_tmp,p_tmp,vr_tmp;
    
    
    
    /* If supersonic, boundary conditions shouldn't matter.  Just copy  */
    if (std::fabs(vr/cs)>1.){
        rho_tmp = den;
        vr_tmp = vr;
        a_tmp = cs;
    ...
    else if (std::fabs(SQR(vr)/2./(bernoulli)) <1e-3){
        /* If velocity is small, solve pressure equlibrium analytically and then add small correction  */
        
        a_tmp = std::sqrt(gm1 * (gm_/r - SQR(l/r)/2. +bernoulli)) ;
        rho_tmp = std::pow(SQR(a_tmp)/gam/kappa,1./gm1);
        vr_tmp = vr * SQR(r_bound/r) * (den/rho_tmp);
        
        a_tmp =  a_tmp - gm1*SQR(vr_tmp)/4./a_tmp;
        rho_tmp = rho_tmp - rho_tmp* 2.* SQR(vr_tmp/a_tmp)/4. ;
        ...
        get_Bondi_boundary(r/r_bound,&rho_tmp,&vr_tmp,&a_tmp,mdot/mdot_unit,kappa/kappa_unit,bernoulli/bernoulli_unit,l/l_kep);
        a_tmp = a_tmp * v_kep;
        rho_tmp = rho_tmp * den;
        vr_tmp = vr_tmp * v_kep;
    ...
    p_tmp = SQR(a_tmp)/gam * rho_tmp;
    
    
    /*Floor pressure and make rho,v consistent with that floor */
    
    if ( p_tmp<1e-10 || isnan(p_tmp) ){
        p_tmp=1e-10;
        rho_tmp = std::pow(p_tmp/kappa,1./(gm1+1.)) ;
        vr_tmp = vr * SQR(r_bound/r) * (den/rho_tmp);
    }
    
    Real dlogp = (p_tmp - press)/press;
    Real dlogr = dr/r_bound;
    Real dlogp_dlogr = dlogp/dlogr;
    
    /*Prevent large increases in pressure */
    /* p \propto r^{-3\gamma} <--> \rho \propto r^{-3}, which is the critical
     power at which M(r) = const.  */
    if (dlogp_dlogr>(gam*3.)){
        dlogp_dlogr = gam*3.;
        Real dp = dlogp_dlogr*dlogr*press;
        p_tmp = press + dp;
        rho_tmp = std::pow(p_tmp/kappa,1./(gm1+1.)) ;
        vr_tmp = vr * SQR(r_bound/r) * (den/rho_tmp);
    ...
    *rho_new = rho_tmp;
    *vr_new = vr_tmp * (vr_tmp<0);
    *press_new = p_tmp;
    *v_th_new = v_th * (r_bound/r);
    *v_phi_new = v_phi * (r_bound/r);
...
/* User Defined x1 inner boundary condition */
void MyBoundary_ix1(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim, FaceField &b,
                    Real time, Real dt, int is, int ie, int js, int je, int ks, int ke){
...
//                    Real r_bound =pmb->pcoord->x1v(is);
//                    Real r_half = (r+r_bound)/2.;
//                    Real dr = r_bound-r;
//                    prim(n,k,j,is-i) = prim(n,k,j,is);
//                    Real theta = pmb->pcoord->x2v(j);
//                    
//                    Real den = prim(IDN,k,j,is);
//                    //Real delta_phi = gm_/r - gm_/r_bound ;
//                    Real a_grav = gm_/SQR(r_bound);
//                    Real v_perp = std::sqrt( SQR(prim(IVY,k,j,is)) + SQR(prim(IVZ,k,j,is)) );
//                    Real cs = std::sqrt((gm1+1.)*prim(IPR,k,j,is)/den );
//                    Real a_grav_eff = a_grav - SQR(v_perp)/r_bound ; 
//                    Real vr =prim(IVX,k,j,is);
//
//                    
//
//
//                    Real rho_tmp,p_tmp,vr_tmp,a_tmp;
//                    Real gam = gm1+1.;
//                     // get_new_prims(r_half,r_half,r_bound,prim(IPR,k,j,is),prim(IDN,k,j,is),prim(IVX,k,j,is),prim(IPR,k,j,is),prim(IDN,k,j,is),prim(IVX,k,j,is),
//                     //   v_perp,&p_tmp,&rho_tmp,&vr_tmp);
//                     // get_new_prims(r,r_half,r_bound,prim(IPR,k,j,is),prim(IDN,k,j,is),prim(IVX,k,j,is),p_tmp,rho_tmp,vr_tmp,
//                     //   v_perp,&p_tmp,&rho_tmp,&vr_tmp);
                    
// //                    if (i==2){
// //                        Real delta_x = (r_bound-r)/4.;
// //                        get_new_prims(r_bound-delta_x,r_bound-delta_x,r_bound,prim(IPR,k,j,is),prim(IDN,k,j,is),prim(IVX,k,j,is),prim(IPR,k,j,is),prim(IDN,k,j,is),prim(IVX,k,j,is),
// //                                      v_perp,&p_tmp,&rho_tmp,&vr_tmp);
// //                        get_new_prims(r_bound-2.*delta_x,r_bound-delta_x,r_bound,prim(IPR,k,j,is),prim(IDN,k,j,is),prim(IVX,k,j,is),p_tmp,rho_tmp,vr_tmp,
// //                                      v_perp,&p_tmp,&rho_tmp,&vr_tmp);
// //                                      
// //                      Real rho_mid,p_mid,vr_mid;
// //                      rho_mid = rho_tmp;
// //                      p_mid = p_tmp;
// //                      vr_mid = vr_tmp;
// //                        get_new_prims(r_bound-3.*delta_x,r_bound-3.*delta_x,r_bound-2.*delta_x,p_mid,rho_mid,vr_mid,p_mid,rho_mid,vr_mid,
// //                                      v_perp,&p_tmp,&rho_tmp,&vr_tmp);
// //                        get_new_prims(r,r_bound-3.*delta_x,r_bound-2.*delta_x,p_mid,rho_mid,vr_mid,p_tmp,rho_tmp,vr_tmp,
// //                                      v_perp,&p_tmp,&rho_tmp,&vr_tmp);
// //                    }
                    
                    
//                     // prim(IPR,k,j,is-i) = p_tmp;
//                     // prim(IVX,k,j,is-i) = vr_tmp;
//                     // prim(IDN,k,j,is-i) = rho_tmp;

                    
//                     //vr = 0;
                    
//                     //Real dp =den*dr * (a_grav_eff - 2.* SQR(vr)/r_bound) ;
//                     //dp = dp / (1.-SQR(vr/cs));

//                     //if (fabs(1. - fabs(vr/cs) ) < 1e-3) dp = den * dr * a_grav;
//                     //Real drho = dp /SQR(cs);
//                     //drho = 0;
//                     //Real dvr =vr * dr*(a_grav_eff - 2.*SQR(cs)/r_bound );
//                     //dvr = dvr/ (SQR(cs)-SQR(vr));
//                     //prim(IPR,k,j,is-i) = prim(IPR,k,j,is) + dp;
//                     //prim(IDN,k,j,is-i) = std::pow(prim(IPR,k,j,is)/kappa,1./(gm1+1.)) ; //prim(IDN,k,j,is) + drho;
//                     //prim(IVX,k,j,is-i) = prim(IVX,k,j,is) + dvr;
//                     //if ( prim(IPR,k,j,is-i)<1e-10 || isnan(prim(IPR,k,j,is-i)) ) prim(IPR,k,j,is-i)=1e-10;
//                     //if ( prim(IDN,k,j,is-i)<1e-8 || isnan(prim(IDN,k,j,is-i)) ) prim(IDN,k,j,is-i)=1e-8;
//                     //if (isnan(prim(IVX,k,j,is-i))) prim(IVX,k,j,is-1) = prim(IVX,k,j,is);

//                     //prim(IDN,k,j,is-i) = std::pow(prim(IPR,k,j,is-i)/kappa,1./(gm1+1.)) ; //prim(IDN,k,j,is) + drho;
//                     //prim(IVX,k,j,is-i) = vr * SQR(r_bound/r) * (den/prim(IDN,k,j,is-i));

//                     if (prim(IVX,k,j,is-i)>0) prim(IVX,k,j,is-i) = 0.;


//                     Real q = 2.; //3./2.;
//                     Real C = -gm_/r_bound + SQR(v_perp)/(2.*q-2.) + SQR(cs)/gm1; 
...
//                     Real RHS =C + gm_/r - SQR(v_perp)/(2.*q-2.) * std::pow(r_bound/r,2.*q-2.);

//                     if (RHS>=0){
//                     a_tmp = std::sqrt(gm1 * (RHS) );
//                     rho_tmp = std::pow(RHS*gm1/gam/kappa,1./gm1);
//                     }
//                     else{
//                       a_tmp = 0.;
//                       rho_tmp = 0.;
...
//                     Real vphi_tmp;
//                     get_torus(r,theta,&vr_tmp,&vphi_tmp,&vr_tmp);
//                     vr_tmp = 0;
//                     p_tmp = SQR(a_tmp)/gam * rho_tmp;
                    
//                     Real dp = p_tmp - prim(IPR,k,j,is) ;
//                     /* Limit dp */
//                     if (fabs(dp/prim(IPR,k,j,is)) >2. ) {
//                         if (dp>0) dp = 2.*prim(IPR,k,j,is);
//                         else dp = -2.*prim(IPR,k,j,is);
                        
//                         p_tmp = prim(IPR,k,j,is) + dp;
//                     }
                    
                    


//                     /* Prevent large changes in density */
//                     //if (rho_tmp/den >10.)rho_tmp = 10.*den;
...
//                     prim(IVX,k,j,is-i) = vr_tmp;
//                     prim(IDN,k,j,is-i) = rho_tmp;
//                     prim(IVY,k,j,is-i) = 0.; //prim(IVY,k,j,is) * r_bound/r;
//                     prim(IVZ,k,j,is-i) = prim(IVZ,k,j,is-i) *std::pow(r_bound/r,q-1.) ;

//                     if ( prim(IPR,k,j,is-i)<1e-10 || isnan(prim(IPR,k,j,is-i)) ) prim(IPR,k,j,is-i)=1e-10;
//                     if ( prim(IDN,k,j,is-i)<1e-7 || isnan(prim(IDN,k,j,is-i)) ) prim(IDN,k,j,is-i)=1e-7;

//                     if (isnan(prim(IVZ,k,j,is-1))) prim(IVZ,k,j,is-1) = 0.;


//                     //prim(IDN,k,j,is-i) =prim(IDN,k,j,is);
//                     prim(IDN,k,j,is-i) = std::pow(prim(IPR,k,j,is-i)/kappa,1./(gm1+1.)) ;


                    // if (fabs(0.12434644/theta-1.)<1e-6 && n==0){
                    //   fprintf(stderr,"theta: %g cond: %g \n C: %g kappa: %g RHS: %g p_tmp: %g rho_tmp: %g \n r_bound: %g r: %g vy: %g vz: %g \n den_0: %g P_0: %g v_0: %g %g %g \n", theta, fabs(0.12434644/theta-1.),C,kappa, RHS, p_tmp ,rho_tmp, r_bound,r,
                    //    prim(IVY,k,j,is-i),prim(IVZ,k,j,is-i), den,prim(IPR,k,j,is), prim(IVX,k,j,is),prim(IVY,k,j,is),prim(IVZ,k,j,is) );
                    // }




//                     fprintf(stderr,"C: %g RHS: %g p_tmp: %g rho_tmp: %g r_bound: %g r: %g vy: %g vz: %g \n", C,RHS, p_tmp ,rho_tmp, r_bound,r,
//                       prim(IVY,k,j,is-i),prim(IVZ,k,j,is-i) );


//                      Real a_bond,rho_bond,v_bond;
// //                     get_Bondi(r,&rho_bond, &v_bond,&a_bond);
//                        get_torus(r,theta,&rho_bond,&v_bond, &a_bond);
// //                     Real gam = gm1+1.;
// //
//                      fprintf(stderr,"r: %g r_bound: %g dr: %g new rho v p : %g %g %g Bondi sol: %g %g %g \n", r, r_bound,dr, prim(IDN,k,j,is-i), 
//                       prim(IVX,k,j,is-i), prim(IPR,k,j,is-i),rho_bond,v_bond,SQR(a_bond)/gam*rho_bond);
// //
                     //if (n == NHYDRO -1) exit(0);



//                    Real kappa = prim(IPR,k,j,is)/std::pow(den,gm1+1.);
//                    
//                    Real mdot = 4. * PI * den * vr * SQR(r_bound);
//                    Real bernoulli = SQR(vr)/2. + SQR(v_perp)/2. + SQR(cs)/gm1 - gm_/r_bound;
//                    Real l = v_perp * r_bound;
...
//                          a_tmp =  a_tmp - gm1*SQR(vr_tmp)/4./a_tmp;
//
//                          //da = d(a^2)/(2a) = gam/(2a) * kappa gm1 rho^(gam-2) drho  = a *gm1 /2 drho/rho  
//                          rho_tmp = rho_tmp - rho_tmp* 2.* SQR(vr_tmp/a_tmp)/4. ;
//
//                          if (isnan(a_tmp)){
//                            a_tmp = 0.;
...
//                    }
//
//                    Real dlogp = (p_tmp - prim(IPR,k,j,is))/(prim(IPR,k,j,is));
//                    Real dlogr = dr/r_bound;
//                    Real dlogp_dlogr = dlogp/dlogr;
//
//                    /*Prevent large increases in pressure */
...
//                    if (dlogp_dlogr>(gam*3.)){
//                          dlogp_dlogr = gam*3.;
//                          Real dp = dlogp_dlogr*dlogr*prim(IPR,k,j,is);
//                          p_tmp = prim(IPR,k,j,is) + dp;
//                          rho_tmp = std::pow(p_tmp/kappa,1./(gm1+1.)) ;
//                          vr_tmp = vr * SQR(r_bound/r) * (den/rho_tmp);
//
//                    }
                    ...
                    Real r = pmb->pcoord->x1v(is-i);
                    Real r_bound =pmb->pcoord->x1v(is);
                    Real rho_tmp,p_tmp,vr_tmp,vphi_tmp,vth_tmp;
                    compute_radial_boundary_values(r,r_bound,prim(IDN,k,j,is),prim(IPR,k,j,is),prim(IVX,k,j,is),
                                                   prim(IVY,k,j,is),prim(IVZ,k,j,is),&rho_tmp,&p_tmp,&vr_tmp,&vth_tmp,
                                                   &vphi_tmp);
                    ...
                    prim(IVX,k,j,is-i) = vr_tmp;
                    prim(IDN,k,j,is-i) = rho_tmp;
                    prim(IVY,k,j,is-i) = vth_tmp;
                    prim(IVZ,k,j,is-i) = vphi_tmp;

                    // Real a_tmp;
                    // Real theta = pmb->pcoord->x2v(j);
                    // get_torus(r,theta,&rho_tmp,&vphi_tmp,&a_tmp);
                    // //get_Bondi(r,&rho_tmp,&vr_tmp,&a_tmp);
                    // Real gam = gm1+1.;
                    // Real dr = r_bound -r;

                    // p_tmp = SQR(a_tmp)/gam * rho_tmp;

                    // fprintf(stderr,"r: %g r_bound: %g dr: %g new rho v p vphi: %g %g %g %g Bondi sol: %g %g %g %g\n", r, r_bound,dr, prim(IDN,k,j,is-i), 
                    //  prim(IVX,k,j,is-i), prim(IPR,k,j,is-i),prim(IVZ,k,j,is-i),rho_tmp,vr_tmp,SQR(a_tmp)/gam*rho_tmp,vphi_tmp);
...
#pragma simd
      for (int i=pmb->is; i<=pmb->ie; ++i) {
          ...
          if (COORDINATE_SYSTEM=="spherical_polar") r= pmb->pcoord->x1v(i);
...
//              }
              ...
              get_cartesian_coords(pmb->pcoord->x1v(i1),pmb->pcoord->x2v(j1),pmb->pcoord->x3v(k1),&x1,&y1,&z1);
              
              Real d1 =cons(IDN,k1,j1,i1);
              Real vx1 =cons(IM1,k1,j1,i1)/d1;
              Real vx2 =cons(IM2,k1,j1,i1)/d1;
              Real vx3 =cons(IM3,k1,j1,i1)/d1;
              Real vmag1 = std::sqrt(SQR(vx1) + SQR(vx2) + SQR(vx3));

              Real th1 = std::acos(z1/r1);
              Real phi1 = std::atan2(y1,x1);
              ...
              Real vr1,vth1,vphi1;
              convert_cartesian_vector_to_spherical_polar(vx1,vx2,vx3,x1,y1,z1,&vr1,&vth1,&vphi1);
                                   
              // Real vr1 = vx1 * x1/r1 + vx2 * y1/r1 * is_2D + vx3 * z1/r1 * is_3D;
              // Real vth1 = vx1 * std::cos(th1)*std::cos(phi1) +vx2 * std::cos(th1)*std::sin(phi1) + vx3 * (-std::sin(th1));
              // Real vphi1 = vx1 * -std::sin(phi1) + vx2 * std::cos(phi1);
              // Real v_perp = std::sqrt( SQR(vx1) + SQR(vx2) + SQR(vx3) - SQR(vr1));


                                   
              Real u1 = cons(IEN,k1,j1,i1) - 0.5 * d1*SQR(vmag1);
              Real p1 = gm1*u1;
              Real dr = r1 - r ;


              Real rho_tmp,p_tmp,vr_tmp,vth_tmp,vphi_tmp;
              compute_radial_boundary_values(r, r1,d1, p1,vr1, vth1, vphi1, &rho_tmp, &p_tmp, &vr_tmp, &vth_tmp, &vphi_tmp);

              Real vx_tmp,vy_tmp,vz_tmp;

              convert_spherical_polar_vector_to_cartesian(vr_tmp, vth_tmp, vphi_tmp, x, y,z, &vx_tmp,&vy_tmp,&vz_tmp);


              // vx_tmp = vr_tmp * std::sin(theta)*std::cos(phi) + vth_tmp * (std::cos(theta)*std::cos(phi)) + vphi_tmp * (-std::sin(phi));
              // vy_tmp = vr_tmp * std::sin(theta)*std::sin(phi) + vth_tmp * (std::cos(theta)*std::sin(phi)) + vphi_tmp * (std::cos(phi));
              // vz_tmp = vr_tmp * std::cos(theta)               + vth_tmp * (-std::sin(theta));

              cons(IDN,k,j,i) = rho_tmp;
              cons(IM1,k,j,i) = rho_tmp * vx_tmp;
              cons(IM2,k,j,i) = rho_tmp * vy_tmp;
              cons(IM3,k,j,i) = rho_tmp * vz_tmp;
              cons(IEN,k,j,i) = 0.5* rho_tmp * ( SQR(vx_tmp) + SQR(vy_tmp) + SQR(vz_tmp) ) + p_tmp/gm1;

              Real a_grav = gm_/SQR(r1);
              ...
              // Real new_p ;
              // new_p = p1 + d1*dr * (a_grav -  SQR(v_perp)/r1 );
              
              // if ( new_p<1e-10 || isnan(new_p) ) new_p=1e-10;
              
              //cons(IEN,k,j,i) = new_p/gm1 + 0.5*d1*SQR(vmag1);
...
//              Real rho_0 = cons(IDN,k,j,i);
//              Real v_10 = cons(IM1,k,j,i)/rho_0;
//              Real v_20 = cons(IM2,k,j,i)/rho_0;
//              Real v_30 = cons(IM3,k,j,i)/rho_0;
//              Real v_0 = std::sqrt((SQR(v_10) + SQR(v_20) + SQR(v_30))); 
//              Real u_0 = cons(IEN,k,j,i) - 0.5*rho_0*SQR(v_0);
//
//              cons(IM1,k,j,i) = 0;
//              cons(IM2,k,j,i) = 0;
//              cons(IM3,k,j,i) = 0;
...
void apply_torus_boundary(MeshBlock *pmb,AthenaArray<Real> &cons){
...
#pragma simd
      for (int i=pmb->is; i<=pmb->ie; ++i) {
          ...
          if (r <= r_inner_boundary_torus || r>= r_outer_boundary_torus){

              
              //bondi_interp(r,&rho,&v,&a);

              if (PROBLEM_TYPE == "torus"){
                get_torus(r,theta,&rho,&v,&a);
              }
              else{
                get_Bondi(r,&rho,&v,&a);
              }
...
//              cons(IM3,k,j,i) = rho * v_init_z;
              ...
              if (PROBLEM_TYPE == "torus"){
                 if (COORDINATE_SYSTEM == "cartesian"){
                     cons(IM1,k,j,i) = rho * v*  -y/s;
                     cons(IM2,k,j,i) = rho * v*  x/s;
                     cons(IM3,k,j,i) = 0;

                 }
                 else if (COORDINATE_SYSTEM == "cylindrical"){
                     cons(IM1,k,j,i) = 0.;
                     cons(IM2,k,j,i) = rho * v;
                     cons(IM3,k,j,i) = 0.;
                 }
                 else if (COORDINATE_SYSTEM == "spherical_polar"){
                    cons(IM1,k,j,i) = 0;
                    cons(IM2,k,j,i) = 0.;
                    cons(IM3,k,j,i) = rho*v;
                 }
            }
...
Real v_of_a( const Real a, const Real r){
    return -qs /SQR(r) * pow(a,(-2./(gm1)));
  }
Real a_of_x(const Real x,const Real r){
    //#v/a = x -> -qs/r * (a) **(-2/(gam-1) -1.)
    Real x_pow = -2./(gm1) -1.;
    return pow( (x*SQR(r)/qs) , (1./x_pow) );
  }

Real v_of_a_Boundary( const Real a, const Real r,const Real mdot, const Real kappa){
    
    Real gam = gm1+1.;
    return mdot/(4.*PI*r*r) * std::pow(gam*kappa/SQR(a), 1./(gam-1.));
}
Real a_of_x_Boundary(const Real x,const Real r,const Real mdot, const Real kappa){
    
    Real gam = gm1+1.;
    return std::pow(fabs(mdot/(4.* PI * r * r *x)),(gam-1.)/(gam+1.)) * pow(gam*kappa,1./(gam+1.));
}
...
static Real MachResidual_Boundary(Real x, Real r, const Real mdot, const Real kappa, const Real bernoulli, const Real l){


    Real a = a_of_x_Boundary(x,r,mdot,kappa);
    Real vr = v_of_a_Boundary(a,r,mdot,kappa);
    return SQR(vr)/2. + SQR(l)/2./SQR(r) + SQR(a)/(gm1) -1./r - bernoulli ;


}

static Real MachBisect_Boundary(Real r, Real x_min, Real x_max,const Real mdot, const Real kappa, const Real bernoulli, const Real l)
{
  // Parameters
  ...
  Real res_min = MachResidual_Boundary(x_min, r,mdot,kappa,bernoulli,l);
  Real res_max = MachResidual_Boundary(x_max, r,mdot,kappa,bernoulli,l);
  ...
  for (int i = 0; i < max_iterations; ++i) {
    ...
    Real res_mid = MachResidual_Boundary(x_mid, r,mdot,kappa,bernoulli,l);
...
void get_Bondi(const Real r, Real *rho, Real *v, Real *a ){
   ...
   Real gam =gm1+1.;
   Real r_scaled = r/r_a;
   qs = 0.25*pow( 2./(5.-3.*gam) ,  (5.-3.*gam)/(2.*gam-2.)   );
   Real mdot  = 4.*PI * qs;
   Real rs = (5.-3.*gam)/4.;
   Real x;
   ...
   *rho = mdot /(4.*PI *-(*v/a_inf)*SQR(r_scaled)) * rho_inf;
}
...
void get_Bondi_boundary(const Real r_scaled, Real *rho_scaled, Real *vr_scaled, Real *a_scaled,const Real mdot,const Real kappa,const Real bernoulli,const Real l){
  ...
  x = MachBisect_Boundary(r_scaled,1e-10,1.,mdot,kappa,bernoulli,l);

  *a_scaled =  a_of_x_Boundary(x,r_scaled,mdot,kappa);
  *vr_scaled = v_of_a_Boundary(*a_scaled,r_scaled,mdot,kappa);
  *rho_scaled = std::fabs(mdot /(4.*PI * (*vr_scaled)*SQR(r_scaled)) );
...
static Real v_phi( const Real s){
    //return 0;
    return 0.5*std::sqrt(gm_/s);
}
void get_torus(const Real r, const Real theta,Real *rho, Real *v, Real *a ){


   Real r_out = 10.;  /* inner boundary of torus */
    Real q = 3./2.; /* \Omega \propto s^{-q} */
    Real C ;
    Real s = r * sin(theta);
    Real kappa = 1.;
    
    C = -gm_/r_out + SQR(v_phi(r_out))/(2.*q-2.);
    
    
   Real gam =gm1+1.;
    

    Real RHS =C + gm_/r - SQR(v_phi(s))/(2.*q-2.);

    if (RHS>=0){
        *a = std::sqrt(gm1 * (RHS) );
        *v = v_phi(s);
        *rho = std::pow(RHS*gm1/gam/kappa,1./gm1);
    }
    else{
        *rho = 0.;
        *v = 0.;
...
// static Real v_phi( const Real R_pmax,const Real q,const Real s){
//     return std::sqrt(gm_/R_pmax) * std::pow(s/R_pmax,-q+1.);
// }
// void get_torus(const Real r, const Real theta,Real *rho, Real *v, Real *a ){


//    Real r_in = .01/5.; //0.1;  /* inner boundary of torus */
//     Real q = 2.; /* \Omega \propto s^{-q} */
//    Real R_pmax = .0037; //0.18;  /* pressure maximum location */
//     Real C ;
//     Real s = r * sin(theta);
//     Real kappa = 1.;

    
//     C = -gm_/r_in + SQR(v_phi(R_pmax,q,r_in))/(2.*q-2.);
    
    
//    Real gam =gm1+1.;
    

//     Real RHS =C + gm_/r - SQR(v_phi(R_pmax,q,s))/(2.*q-2.);
...
Real radial_profile(MeshBlock *pmb, int iout){
    ...
    r_dump_min = pmb->r_inner_boundary;
...
void Mesh::InitUserMeshData(ParameterInput *pin)
{
    ...
    if (COORDINATE_SYSTEM == "cartesian"){
        AllocateUserHistoryOutput(N_r*(NHYDRO+N_user_vars +1));
        for (int i = 0; i<N_r*(NHYDRO+N_user_vars +1); i++){
...
void MeshBlock::InitUserMeshBlockData(ParameterInput *pin){
    ...
    PROBLEM_TYPE = pin->GetOrAddString("problem","problem_type","torus");
    ...
    Real N_cells_per_inner_boundary = pin->GetOrAddReal("problem", "boundary_radius", 2.0);

    if (block_size.nx3>1)       r_inner_boundary = N_cells_per_inner_boundary * std::max(std::max(dx_min,dy_min),dz_min); // r_inner_boundary = 2*sqrt( SQR(dx_min) + SQR(dy_min) + SQR(dz_min) );
    else if (block_size.nx2>1)  r_inner_boundary = N_cells_per_inner_boundary * std::max(dx_min,dy_min); //2*sqrt( SQR(dx_min) + SQR(dy_min)               );
    else                        r_inner_boundary = N_cells_per_inner_boundary * dx_min;
    ...
    //r_inner_boundary_torus = r_inner_boundary;
    r_inner_boundary_torus = pin->GetOrAddReal("problem", "inner_radius", r_inner_boundary);
    r_outer_boundary_torus = pin->GetOrAddReal("problem", "outer_radius", pcoord->pmy_block->pmy_mesh->mesh_size.x1max - 2.*DX);

    if (r_inner_boundary_torus<r_inner_boundary ){
      fprintf(stderr,"Warning, inner boundary for dirichlet conditions is less than two cells \n r_inner_boundary_torus: %g\n", r_inner_boundary_torus);
    }
    if (r_outer_boundary_torus>pcoord->pmy_block->pmy_mesh->mesh_size.x1max - 2.*DX){
      fprintf(stderr,"Warning, outer boundary for dirichlet conditions is less than two cells \n r_outer_boundary_torus: %g\n", r_outer_boundary_torus);
    }
    //r_outer_boundary_torus = pcoord->pmy_block->pmy_mesh->mesh_size.x1max - 2.*DX ;


    if (COORDINATE_SYSTEM == "cylindrical" || COORDINATE_SYSTEM == "spherical_polar"){
//      r_inner_boundary_torus = pcoord->pmy_block->block_size.x1min + 2.*(pcoord->pmy_block->block_size.x1max -pcoord->pmy_block->block_size.x1min)/pcoord->pmy_block->block_size.nx1;
//      r_outer_boundary_torus = pcoord->pmy_block->block_size.x1max - 2.*(pcoord->pmy_block->block_size.x1max -pcoord->pmy_block->block_size.x1min)/pcoord->pmy_block->block_size.nx1;
        
        r_inner_boundary_torus = pin->GetOrAddReal("problem", "inner_radius", pcoord->pmy_block->block_size.x1min + 2.*(pcoord->pmy_block->block_size.x1max -pcoord->pmy_block->block_size.x1min)/pcoord->pmy_block->block_size.nx1);
        r_outer_boundary_torus =pin->GetOrAddReal("problem", "outer_radius", pcoord->pmy_block->block_size.x1max - 2.*(pcoord->pmy_block->block_size.x1max -pcoord->pmy_block->block_size.x1min)/pcoord->pmy_block->block_size.nx1);
    }
...
void MeshBlock::ProblemGenerator(ParameterInput *pin)
{
  ...
  // convert_cartesian_vector_to_spherical_polar(1,2.,3.,1.,2.,3.,&tmp1,&tmp2,&tmp3);
  //   fprintf(stderr,"cart to spherical: %g %g %g \n",tmp1,tmp2,tmp3);

  // convert_spherical_polar_vector_to_cartesian(1.,2.,3.,1.,2.,3.,&tmp1,&tmp2,&tmp3);

  //     fprintf(stderr,"spherical to cart: %g %g %g \n",tmp1,tmp2,tmp3);
...
/* Torus */
      ...
      if (PROBLEM_TYPE == "torus"){
        if (COORDINATE_SYSTEM == "cartesian"){
          phydro->u(IM1,k,j,i) = rho * v * -y/s;
          phydro->u(IM2,k,j,i) = rho * v * x/s;
          if ((block_size.nx3)>1) phydro->u(IM3,k,j,i) = 0;
