...
Mesh::Mesh(ParameterInput *pin, int mesh_test) {
  ...
  UserRadSourceTerm_=NULL;
  UserEMFSourceTerm_= NULL;
  StarUpdateFunction_=NULL;
  UserTimeStep_=NULL;
...
//    gflag=2;
  ...
  if (resfile.Read_at_all(mbdata, datasize, nb, headeroffset+nbs*datasize) !=
    ...
    msg << "### FATAL ERROR in Mesh constructor" << std::endl
        << "The restart file is broken or input parameters are inconsistent. (1)"
        << std::endl;
    throw std::runtime_error(msg.str().c_str());
  }
  for (i=nbs;i<=nbe;i++) {
  ...
  if (datasize!=pblock->GetBlockSizeInBytes()) {
    msg << "### FATAL ERROR in Mesh constructor" << std::endl
        << "The restart file is broken or input parameters are inconsistent. (2)"
        << "datasize: " << datasize << std::endl
        << "blocksize:" << pblock->GetBlockSizeInBytes()
        << std::endl;
    throw std::runtime_error(msg.str().c_str());
  }
...
void Mesh::EnrollUserExplicitEMFSourceFunction(EMFSrcTermFunc_t my_func)
{
...
void Mesh::EnrollUserStarUpdateFunction(SrcTermFunc_t my_func)
{
...
#pragma omp for private(pmb,pbval,phydro,pfield)
    for (int i=0; i<nmb; ++i) {
      ...
      //fprintf(stderr,"Before: rho: %g rho_u0: %g \n",phydro->w(IDN,10,10,10), phydro->u(IDN,10,10,10));
      pmb->peos->ConservedToPrimitive(phydro->u, phydro->w1, pfield->b,
                                      phydro->w, pfield->bcc, pmb->pcoord,
                                      il, iu, jl, ju, kl, ku);
      //fprintf(stderr,"After: rho: %g rho_u0: %g \n",phydro->w(IDN,10,10,10), phydro->u(IDN,10,10,10));
