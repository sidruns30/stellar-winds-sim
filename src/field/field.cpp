...
Field::Field(MeshBlock *pmb, ParameterInput *pin) {
  ...
  if (MAGNETIC_FIELDS_ENABLED) {
    ...
    if (ALLOCATE_FIELD_BOUNDARY_ARRAYS){
      b_bound.x1f.NewAthenaArray( ncells3   , ncells2   ,(ncells1+1));
      b_bound.x2f.NewAthenaArray( ncells3   ,(ncells2+1), ncells1   );
      b_bound.x3f.NewAthenaArray((ncells3+1), ncells2   , ncells1   );
    }
...
Field::~Field() {
  ...
  if (ALLOCATE_FIELD_BOUNDARY_ARRAYS){
    b_bound.x1f.DeleteAthenaArray();
    b_bound.x2f.DeleteAthenaArray();
    b_bound.x3f.DeleteAthenaArray();
  }
