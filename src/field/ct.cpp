...
#pragma omp simd
      for (int i=is; i<=ie+1; ++i) {
        b_out.x1f(k,j,i) -= wght*
          ...
          //  if (std::isnan(b_out.x1f(k,j,i))) {
          //   fprintf(stderr,"b_out x1f e3 \n area: %g lenp1: %g e3 jp1: %g len: %g e3; %g \n",area(i),len_p1(i),e3(k,j+1,i),len(i),e3(k,j,i));
          //   exit(0);
          // }
      }
