...
public:
  ...
  enum TaskStatus EMFSourceTerms(MeshBlock *pmb, int step);
  enum TaskStatus RadSourceTerms(MeshBlock *pmb, int step);
  enum TaskStatus StarUpdate(MeshBlock *pmb, int step);
...
namespace HydroIntegratorTaskNames {
  ...
  const uint64_t SRCTERM_EMF=1LL<<20;
  ...
  const uint64_t STAR_UPDATE=1LL<<56;
