...
TimeIntegratorTaskList::TimeIntegratorTaskList(ParameterInput *pin, Mesh *pm)
  ...
  {using namespace HydroIntegratorTaskNames; // NOLINT (build/namespace)
    ...
    AddTimeIntegratorTask(SRCTERM_HYD,(INT_HYD|STAR_UPDATE));

    if (MAGNETIC_FIELDS_ENABLED){ //MHD
    }
    else{ //Hydro
      AddTimeIntegratorTask(SRCTERM_RAD,SRCTERM_HYD);
    }
    AddTimeIntegratorTask(SEND_HYD,SRCTERM_RAD);
    AddTimeIntegratorTask(RECV_HYD,START_ALLRECV);
      AddTimeIntegratorTask(STAR_UPDATE,START_ALLRECV);
    ...
    if (MAGNETIC_FIELDS_ENABLED) { // MHD
      ...
      AddTimeIntegratorTask(SRCTERM_EMF,(CALC_FLDFLX|STAR_UPDATE));
      AddTimeIntegratorTask(SEND_FLDFLX,SRCTERM_EMF);
      ...
      AddTimeIntegratorTask(SRCTERM_RAD,(INT_FLD|SRCTERM_HYD));
      AddTimeIntegratorTask(SEND_FLD,SRCTERM_RAD);
    ...
    // everything else
    AddTimeIntegratorTask(PHY_BVAL,(CON2PRIM|SRCTERM_RAD));
...
enum TaskStatus TimeIntegratorTaskList::EMFSourceTerms(MeshBlock *pmb, int stage)
{
  Hydro *ph=pmb->phydro;
  Field *pf=pmb->pfield;

  // return if there are no source terms to be added
  if (pf->emf_sourceterms_defined == false) return TASK_NEXT;
...
enum TaskStatus TimeIntegratorTaskList::RadSourceTerms(MeshBlock *pmb, int stage)
{
  ...
  // return if there are no source terms to be added
  if (ph->psrc->rad_sourceterms_defined == false) return TASK_NEXT;
  ...
  if (stage <= nstages) {
    ...
    ph->psrc->AddRadSourceTerms(t_start_stage,dt,ph->w,ph->w);
...
enum TaskStatus TimeIntegratorTaskList::StarUpdate(MeshBlock *pmb, int stage)
{
  ...
  // return if there are no source terms to be added
  if (ph->psrc->star_update_defined == false) return TASK_NEXT;
  ...
  if (stage <= nstages) {
    ...
    ph->psrc->DoStarUpdate(t_start_stage,dt,ph->flux,ph->w,pf->bcc,ph->u);
  } 
  else {
    return TASK_FAIL;
  }
