    ...
    for (int j=jl; j<=ju; ++j) {
      ...
      for (int i=il; i<=iu; ++i) {
        ...
        if (not (wgas_rel_true > 0.0 and wgas_rel_true < max_wgas_rel)) {
          ...
          //fprintf(stderr,"error in wgas_rel_true wgas: %g\n wgas_rel_init: %g rho_old: %g pgas_old: %g uu_old %g %g %G",
          //   wgas_rel_true, wgas_rel_init, rho_old,pgas_old,uu1_old,uu2_old,uu3_old);
          // fprintf(stderr,"error in wgas_rel_true wgas: %g\n wgas_rel_init: %g rho_old: %g pgas_old: %g uu_old %g %g %g \n r: %g \n",
          //   wgas_rel_true, wgas_rel_init, rho_old,pgas_old,uu1_old,uu2_old,uu3_old);
          //exit(0);
        }
        ...
        if (std::isnan(gamma_rel) or not std::isfinite(gamma_rel) or gamma_rel < 1.0) {
          ...
          //fprintf(stderr,"error in gamma_sq \n");
        }
        // unused:
        ...
        if (gamma_rel > gamma_max_) {
          ...
          uu3 *= factor;
          fixed_(k,j,i) = true;
          //fprintf(stderr,"ceiling on v...gamma_rel: %g \n", gamma_rel);
        }
