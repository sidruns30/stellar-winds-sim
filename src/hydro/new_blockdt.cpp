        ...
        for (int i=is; i<=ie; ++i) {
          ...
          if (MAGNETIC_FIELDS_ENABLED) {
            ...
            dt3(i) /= (fabs(wi[IVZ]) + cs);
                        ...
                        /* If the cell falls inside the inner boundary, than do not worry about its cell crossing time */
            if (COORDINATE_SYSTEM == "cartesian"){
              Real r = std::sqrt( SQR(pmb->pcoord->x1v(i)) + SQR(pmb->pcoord->x2v(j))*(pmb->block_size.nx2>1) + SQR(pmb->pcoord->x3v(k))*(pmb->block_size.nx3>1) );
