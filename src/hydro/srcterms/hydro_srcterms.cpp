...
HydroSourceTerms::HydroSourceTerms(Hydro *phyd, ParameterInput *pin) {
  ...
  if (gm_ != 0.0) {
    ...
    || (COORDINATE_SYSTEM == "cartesian")) {
      ...
      msg << "### FATAL ERROR in HydroSourceTerms constructor" << std::endl
          ...
          << "or cartesian coordinates." << std::endl
  ...
  if(StarUpdateFunction != NULL) star_update_defined = true;
}
...
void HydroSourceTerms::AddRadSourceTerms(const Real time, const Real dt, const AthenaArray<Real> &prim_old,
      AthenaArray<Real> &prim)
{
  ...
  if (UserRadSourceTerm != NULL)
    UserRadSourceTerm(pmb, time,dt,prim,prim);
     ...
     const AthenaArray<Real> &bcc, AthenaArray<Real> &cons)
{
  ...
  //  user-defined source terms
  if (StarUpdateFunction != NULL)
    StarUpdateFunction(pmb, time,dt,prim,bcc,cons);
